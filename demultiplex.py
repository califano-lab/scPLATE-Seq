...
prefix=''
#define arguments
opt, args = getopt.getopt(sys.argv[1:], 'd:i:l:u:s:b:p', ['workDir=',
                                                    'barcodeInfo=',
                                                    'barcodeLen=',
                                                    'umiLen=',
                                                    'seqFastq=',
...
print("barcodeLen   : %s" % barcodeLen)
print("umiLen       : %s" % umiLen)
...
while True:
    ...
    bl4 = bf.readline()
    sl4 = sf.readline()
    if bl2[0:int(barcodeLen)] in BARCODE:
        barcode_dict[SAMPLE[BARCODE.index(bl2[0:int(barcodeLen)])]].append(sl1.rstrip()+'\n'+bl2[int(barcodeLen):int(barcodeLen)+int(umiLen)].rstrip()+sl2.rstrip()+'\n'+sl3.rstrip()+'\n'+bl4[int(barcodeLen):int(barcodeLen)+int(umiLen)].rstrip()+sl4.rstrip()+'\n')
bf.close()
sf.close()
#########
#.fastq files
